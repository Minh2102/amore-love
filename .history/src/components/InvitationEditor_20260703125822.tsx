...
import { WeddingInvitation, LoveStoryEvent, MenuItem, Template } from "../types";
import { LanguageCode, translations } from "../translations";
import ShareModal from "./ShareModal";

interface InvitationEditorProps {
  selectedTemplateId: string | null;
  initialInvitation?: WeddingInvitation | null;
  onPreviewInvitation: (slug: string, previewData?: WeddingInvitation | null) => void;
  lang: LanguageCode;
}
...
const DEFAULT_INVITATION: WeddingInvitation = {
  ...
  musicUrl: "/uploads/dam-cuoi-nha-hong-thanh-dj-mie.mp3",
  musicTitle: "Đám Cưới Nha - Hồng Thanh ft. Mie",
...
export default function InvitationEditor({ selectedTemplateId, initialInvitation, onPreviewInvitation, lang }: InvitationEditorProps) {
  const t = translations[lang];

  // Editor configuration states
  const [form, setForm] = useState<WeddingInvitation>(initialInvitation || DEFAULT_INVITATION);
  ...
  const builtInSongs = [
    {
      title: "Đám Cưới Nha - Hồng Thanh ft. Mie",
      url: "/uploads/dam-cuoi-nha-hong-thanh-dj-mie.mp3"
    },
    {
      title: "Em Đồng Ý (I Do) - Đức Phúc ft. 911",
      url: "/uploads/em-dong-y-i-do-duc-phuc-911.mp3"
    },
    {
      title: "Em Sẽ Là Cô Dâu - Minh Vương M4U ft. Huy Cung",
      url: "/uploads/em-se-la-co-dau-minh-vuong-m4u-huy-cung.mp3"
    },
    {
      title: "Làm Vợ Anh Nhé - Chi Dân",
      url: "/uploads/lam-vo-anh-nhe-chi-dan.mp3"
    },
    {
      title: "Ta Là Của Nhau (Remix) - Ân Thiên Vỹ, Võ Lê Mi",
      url: "/uploads/ta-la-cua-nhau-remix-an-thien-vy-vo-le-mi.mp3"
    },
    {
      title: "Xứng Đôi Cưới Thôi - Lê Thiện Hiếu",
      url: "/uploads/xung-doi-cuoi-thoi-le-thien-hieu.mp3"
    },
    {
      title: "Yêu Em Hơn Mỗi Ngày - Hoàng Hải Dương",
      url: "/uploads/yeu-em-hon-moi-ngay-hoang-hai-duong.mp3"
    }
  ];
  ...
  const uploadFileToApi = async (file: File) => {
    ...
    formData.append("name", file.name.split(".")[0] || "file");
  ...
  const handleAudioUpload = async (file: File) => {
    ...
    try {
      ...
      updateForm({
        ...
        musicTitle: file.name.replace(/\.[^/.]+$/, "") || form.musicTitle || "Nhạc nền"
      });
    } catch (error: any) {
      alert("Không thể tải nhạc lên: " + (error.message || error));
    } finally {
  ...
  }, [selectedTemplateId, initialInvitation]);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (savedData && typeof savedData === "object") {
        setForm((prev) => ({ ...prev, ...savedData } as WeddingInvitation));
      }
  ...
  return (
    <div id="editor-layout" className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
      ...
      <div id="editor-sections" className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
        ...
        <div className="lg:col-span-3 space-y-2 bg-white p-4 rounded-3xl border border-stone-200 lg:sticky lg:top-24 lg:self-start lg:max-h-[calc(100vh-6rem)] lg:overflow-y-auto">
          <button
            ...
            <div className="space-y-6">
              ...
              <div className="border-t border-stone-100 pt-6 mt-8 space-y-5">
                ...
                <div className="border-t border-stone-200 pt-5">
                  <span className="text-xs font-semibold uppercase tracking-wider text-stone-500">Nhạc tích hợp sẵn</span>
                  <div className="mt-3 grid grid-cols-1 sm:grid-cols-2 gap-3">
                    {builtInSongs.map((song) => {
                      const isSelected = form.musicUrl === song.url;
                      return (
                        <button
                          key={song.url}
                          type="button"
                          ...
                          className={`w-full rounded-2xl border px-4 py-3 text-left text-xs transition-all ${isSelected ? 'border-rose-400 bg-rose-50 text-stone-900 shadow-sm' : 'border-stone-200 bg-white text-stone-700 hover:border-rose-300 hover:bg-rose-50'}`}
                        >
                          <div className="flex items-center justify-between gap-3">
                            <div className="font-semibold text-sm">{song.title}</div>
                            {isSelected && (
                              <span className="text-[10px] uppercase tracking-widest text-rose-500 font-bold">Đã chọn</span>
                            )}
                          </div>
                          <div className="text-[10px] text-stone-500 mt-1 truncate">{song.url}</div>
                        </button>
                      );
                    })}
                  </div>
                ...
                <div className="mt-4 border-t border-stone-200 pt-4">
                  <span className="text-xs font-semibold uppercase tracking-wider text-stone-500">Tải nhạc cá nhân</span>
                  <label className="mt-2 inline-flex items-center gap-2 px-4 py-2.5 bg-rose-500 hover:bg-rose-600 text-white rounded-xl text-xs font-bold cursor-pointer select-none transition-colors">
                    {uploadingAudio ? (
                      <RefreshCw className="h-4 w-4 animate-spin" />
                    ) : (
                      <Upload className="h-4 w-4" />
                    )}
                    <span>{uploadingAudio ? "Đang tải nhạc..." : "Tải bài hát từ thiết bị (.mp3)"}</span>
                    <input
                      type="file"
                      accept="audio/*"
                      className="hidden"
                  ...
                  <p className="mt-2 text-[11px] text-stone-500">Sau khi tải lên, đường link sẽ được lưu tự động và bạn có thể dùng đường dẫn này làm nhạc nền.</p>
                </div>
