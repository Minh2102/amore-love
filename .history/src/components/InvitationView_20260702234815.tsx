...
import { 
  ...
  ChevronRight, Volume2, VolumeX, Sparkles, AlertCircle, CheckCircle2, X 
} from "lucide-react";
...
export default function InvitationView({ slug, lang }: InvitationViewProps) {
  ...
  const loadData = () => {
    ...
    fetch(`/api/invitations/${slug}/dashboard`)
      ...
      .catch((err) => {
        ...
        setLoadError(err.message || "Lỗi tải thiệp cưới. Vui lòng thử lại.");
        setLoading(false);
      });
  };
  ...
  useEffect(() => {
    ...
    // Create new audio instance only if musicUrl exists
    if (invite?.musicUrl) {
      try {
        audioRef.current = new Audio(invite.musicUrl);
    ...
    // Cleanup on unmount or when invite changes
    return () => {
      if (audioRef.current) {
        audioRef.current.pause();
        audioRef.current.src = "";
  ...
  const toggleMusic = () => {
    ...
    if (isPlayingMusic) {
      ...
      audioRef.current.play().catch((err) => console.log("User gesture required for audio playback:", err));
    }
  ...
  const handleSubmitRSVP = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setIsToastOpen(true);
      setShowConfetti(true); // Trigger celebratory confetti burst!
      // Reset form fields
      setGuestName("");
      setPhone("");
      setGreetings("");
      ...
      setRsvpStatus({ type: "error", msg: "Gửi phản hồi thất bại. Vui lòng thử lại sau." });
    }
  };
                  ...
                  onClick={() => {
                    ...
                    if (audioRef.current) {
                      audioRef.current.play().catch((err) => {
                        console.log("Audio play failed on gesture transition:", err);
                      });
                    }
                  }}
