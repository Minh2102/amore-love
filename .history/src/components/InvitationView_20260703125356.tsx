...
export default function InvitationView({ slug, lang, previewInvite }: InvitationViewProps) {
  ...
  const handleCopyLiveLink = async () => {
    try {
      ...
      if (isPreviewMode && invite?.createdAt === "") {
        alert("Đây là chế độ xem trước chưa lưu. Đường link chỉ hoạt động khi bạn đã lưu thiệp.");
      }
    } catch (err) {
      ...
      {isPreviewMode && (
        <div className="mx-auto max-w-6xl rounded-3xl border border-amber-200 bg-amber-50/90 text-amber-900 px-4 py-3 mb-4 mt-4 shadow-sm text-xs sm:text-sm flex flex-col sm:flex-row items-center justify-between gap-3">
          <div className="flex-1">
            <span className="font-semibold">Chế độ xem trước thiệp chưa lưu</span>
            <p className="mt-1 text-amber-800/80">Đường link thật chỉ hoạt động khi bạn đã lưu thiệp. Đây là bản xem trước dựa trên dữ liệu hiện tại trong trình chỉnh sửa.</p>
          </div>
          <button
            type="button"
