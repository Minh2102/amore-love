...
export default function InvitationView({ slug, lang, previewInvite }: InvitationViewProps) {
  ...
  const isPreviewMode = Boolean(previewInvite);
  const isUnsavedPreview = isPreviewMode && !invite?.createdAt;
  const liveInviteLink = `${window.location.origin}/?id=${slug}`;
  ...
  const handleCopyLiveLink = async () => {
    try {
      ...
      if (isUnsavedPreview) {
        alert("Đây là chế độ xem trước chưa lưu. Đường link live chỉ hoạt động khi bạn đã lưu thiệp.");
      }
    } catch (err) {
  ...
  return (
    <div id="invitation-view-root" className={`min-h-screen ${containerBgClass}`}>
      ...
      {isPreviewMode && (
        <div className="mx-auto max-w-6xl rounded-3xl border border-amber-200 bg-amber-50/90 text-amber-900 px-4 py-3 mb-4 mt-4 shadow-sm text-xs sm:text-sm flex flex-col sm:flex-row items-center justify-between gap-3">
          <div className="flex-1">
            <span className="font-semibold">Chế độ xem trước thiệp</span>
            <p className="mt-1 text-amber-800/80">
              {isUnsavedPreview
                ? "Đây là bản xem trước chưa lưu. Đường link live chỉ hoạt động khi bạn lưu thiệp trên trang quản trị."
                : "Bạn đang xem trước phiên bản thiệp đã lưu. Đường link live đã sẵn sàng để chia sẻ."}
            </p>
          </div>
          <button
            type="button"
            onClick={handleCopyLiveLink}
            className="rounded-full bg-amber-500 px-4 py-2 text-white text-[11px] font-bold uppercase tracking-[0.15em] hover:bg-amber-600 transition-colors"
          >
            {liveLinkCopied ? "Đã sao chép" : "Sao chép link live"}
          </button>
        </div>
      )}
